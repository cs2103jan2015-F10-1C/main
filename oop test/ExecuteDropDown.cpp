...
string ExecuteDropDown::execute(Storage& _storage, ExtDataBase extdb) {
	...
	if (userInput == "Display All Tasks"){
		return displayAllTask(_storage);
	}
	else if (userInput == "Tasks to be done today"){
		return getTodayTask(_storage);
	}
	else if (userInput == "Sort by Date"){
		return sortTasksByDate(_storage);
	}
	else if (userInput == "Sort by Priority"){
		return sortTasksPriority(_storage);
	}
	else if (userInput == "Display Unbounded Tasks"){
		return getUnboundedTasks(_storage);
	}
	else{
		return "";
	}
...
string ExecuteDropDown::displayAllTask(Storage& _storage){
	ostringstream oss;
...
string ExecuteDropDown::getTodayTask(Storage& _storage){
	...
	for (size_t i = 0; i < _size; i++, iter++){
		if (iter->getDate() == currentDate){
			oss << j << "." << getAllInfoOfOneTask(iter) << "\r\n";
			j++;
		}
	}
...
string ExecuteDropDown::getTodayDate(){
	time_t rawTime;
...
string ExecuteDropDown::getAllInfoOfOneTask(list<StickyNote>::iterator iter){
...
string ExecuteDropDown::sortTasksByDate(Storage& _storage){
...
void ExecuteDropDown::getFutureTasks(vector<list<StickyNote>::iterator> &futureTasks, Storage& _storage){
...
string ExecuteDropDown::sortTasksPriority(Storage& _storage){
...
string ExecuteDropDown::getUnboundedTasks(Storage& _storage){
	list<StickyNote>::iterator iter = _storage.getIter();
	size_t _size = _storage.getSize();
	ostringstream oss;
	int j = 1;
